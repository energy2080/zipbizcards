...
NSString *zipbizcards$ModuleRequireFormat = @"(function(exports){"
...
@implementation zipbizcardsObject
...
{
  // create zipbizcards global object
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

  // Load the "zipbizcards" object into the global scope
  NSString *basePath = (url == nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
  _zipbizcards = [[zipbizcardsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

  TiContextRef jsContext = [kroll context];
  TiValueRef tiRef = [KrollObject toValue:kroll value:_zipbizcards];

  NSString *_zipbizcardsNS = [NSString stringWithFormat:@"T%sanium", "it"];
  TiStringRef prop = TiStringCreateWithCFString((CFStringRef)_zipbizcardsNS);
  ...
  if (preload != nil) {
    for (NSString *name in preload) {
      KrollObject *ti = (KrollObject *)[_zipbizcards valueForKey:name];
...
- (KrollWrapper *)loadCommonJSModule:(NSString *)code withSourceURL:(NSURL *)sourceURL
{
  ...
  NSString *js = [[NSString alloc] initWithFormat:zipbizcards$ModuleRequireFormat, dirname, filename, code];
...
- (KrollWrapper *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
  ...
  if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
    @throw [NSException exceptionWithName:@"org.zipbizcards.kroll"
