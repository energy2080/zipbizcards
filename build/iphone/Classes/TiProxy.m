...
//Rare exceptions to indicate a bug in the _zipbizcards code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString *)qualifiedName withProperties:(NSDictionary *)properties inContext:(id<TiEvaluator>)context
{
  ...
  if (proxyClass == nil) {
    NSString *_zipbizcards = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium."];
    if ([qualifiedName hasPrefix:_zipbizcards]) {
    ...
    if (proxyClass == nil) {
      ...
      @throw [NSException exceptionWithName:@"org.zipbizcards.module"
